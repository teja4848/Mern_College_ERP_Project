...
// Redux action for sending OTP to student
...
// Redux action for submitting OTP and new password for student
export const submitOTPStudent = (credentials) => {
  return async (dispatch) => {
    try {
      const { data } = await axios.post("/api/student/postOTP", credentials);
      
      // Dispatch a success action (optional, based on your state management)
      dispatch({ type: 'PASSWORD_UPDATE_SUCCESS', payload: data });

      // Notify the user (consider using a more user-friendly method than alert)
      ...
      const errorPayload = err.response ? err.response.data : 'Something went wrong';
      dispatch({
        type: 'SET_ERRORS',
