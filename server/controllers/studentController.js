...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    const helper = async () => {
      student.otp = ""; // Clear the OTP
      await student.save();
      console.log(`OTP cleared for student: ${student.email}`);
    }; 
...
exports.updateProfile = async (req, res, next) => {
  try {
    const { email, studentMobileNumber, fatherName, fatherMobileNumber, registrationNumber } = req.body;
    ...
    // Prepare updated data with fallback to current values
    const updatedData = {
      email: email || student.email,
      fatherName: fatherName || student.fatherName,
      fatherMobileNumber: fatherMobileNumber || student.fatherMobileNumber,
    };

    //console.log("Current studentMobileNumber:", studentMobileNumber);

    // Validate studentMobileNumber and only update if it's valid (10 digits, numeric)
    if (studentMobileNumber && /^[0-9]{10}$/.test(studentMobileNumber)) {
      updatedData.studentMobileNumber = studentMobileNumber;
    } else {
      console.log("studentMobileNumber is invalid or not provided.");
    }
    ...
    // Handle file-based avatar upload
    if (req.file) {
    ...
    // Handle base64-based avatar upload
      ...
      // Save the decoded base64 image to the file system
      fs.writeFileSync(filePath, Buffer.from(base64Data, 'base64'));

      // Construct the URL for accessing the saved image
    ...
    // Log the updated data before applying it to the database
